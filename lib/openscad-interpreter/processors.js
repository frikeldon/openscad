...
function getBuiltinModuleParameters (parameterDefinitions, unnamedParameters, parameters) {
  const params = []
  ...
  for (let index = 0; index < parameters.length; index += 1) {
    ...
    } else if (annonymous < unnamedParameters.length) {
      name = unnamedParameters[annonymous]
      annonymous += 1
    } else {
      continue
    }
  ...
  function moduleCall2 (environment, node) {
    ...
    if (module.method) {
      const params = module.parameters
        ? getBuiltinModuleParameters(module.parameters, module.unnamedParameters, parameters)
        : parameters
